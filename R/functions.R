...
makeCompTable <- function(d1, d2) {
  percComp <- function(x) round(mean(x, na.rm = TRUE)*100, 0)
  tibble(
    Wave =
      c(rep(1, 8), 2, 2),
    Game = 
      c("Stag Hunt Game", "Trust Game", "Public Goods Game",
        "Dictator Game", "Ultimatum Game", "Third-Party Punishment Game",
        "Second-Party Punishment Game", "Stag Hunt Game with Punishment",
        "BEAST", "Rule Following Task"),
    `Answered question correctly (%)` =
      c(percComp(d1$egame.cmpSH.T10),
        percComp(d1$egame.cmpTG.T10),
        percComp(d1$egame.cmpPGG.T10),
        percComp(d1$egame.cmpDG.T10),
        percComp(d1$egame.cmpUG.T10),
        percComp(d1$egame.cmpTPP.T10),
        percComp(d1$egame.cmpSPP.T10),
        percComp(d1$egame.cmoSHP.T10),
        percComp(d2$egame.cmpBEAST.T11),
        percComp(d2$egame.cmpRFT.T11))
  )
}

makeSamplePlot <- function(d1) {
  # age
  fig.a <-
    d1 %>% drop_na(Age.T10) %>%
  ...
  out <- ggdraw(out)
  # save plot
  ggsave(out, file = "figures/samplePlot.pdf", 
         height = 6, width = 9, device = cairo_pdf)
  return(out)
}
...
runPCA <- function(d1, extraGames, nfactors, rotate) {
  cols <- c(
    "egame.DG.T10.NA",
    "egame.TG1.T10.NA",
    "egame.TG2.T10.NA",
...
makeCorPcaPlot <- function(cor, pca1.2, pca2.2) {
  ...
  # save
  ggsave(out, file = "figures/corPcaPlot.pdf", height = 5, width = 8)
  return(out)
}

makeScreePlot <- function(pca1.1, pca2.1) {
  ...
  # save
  ggsave(out, file = "figures/screePlot.pdf", height = 4, width = 6)
  return(out)
}
...
runSEM <- function(d1, controls = "", estimator = "DWLS") {
  model <- '# measurement model
            ...
            egame.PGG.T10 + egame.DG.T10 + egame.TG1.T10 + egame.TG2.T10 + egame.SH.T10 + egame.UG2.T10 + egame.TPP2.T10 + egame.SPP3.T10 + egame.SHP3.T10 ~ egame.cmpOverall.T10
            # regressions
            coop ~ a1*SDO.T10.c + RWA.T10.c'
  model <- paste0(model, 
                  controls, 
                  "\n            pun  ~ a2*SDO.T10.c + RWA.T10.c", 
                  controls,
...
makeCoopPunPlot <- function(d1, sem1) {
  ...
  out <- plot_grid(pA, pB, nrow = 1, labels = letters[1:2])
  # save
  ggsave(out, file = "figures/coopPunPlot.pdf", height = 4, width = 8)
  return(out)
}

makeSEMtable <- function(sem7) {
  parameterEstimates(sem7) %>%
    filter(op == "~" & rhs != "egame.cmpOverall.T10") %>%
    select(est:pvalue) %>%
    mutate(
      DV = c("Cooperation", rep("", 8), "Punishment", rep("", 8)),
      IV = rep(c("SDO", "RWA", "Age", "Gender (Male = 1)", "Ethnicity (Pakeha)", "Education", "SES", "Deprivation", "Religious (Yes = 1)"), 2)
    ) %>%
    select(DV, IV, everything()) %>%
    rename(Estimate = est, SE = se, p = pvalue)
}

semiPartialR1 <- function(d1, prevModel, controls, dv, replace, with) {
  model <- '# measurement model
            coop =~ egame.PGG.T10 + egame.DG.T10 + egame.TG1.T10 + egame.TG2.T10 + egame.SH.T10
            pun  =~ egame.UG2.T10 + egame.TPP2.T10 + egame.SPP3.T10 + egame.SHP3.T10
            # control for comprehension
...
fitIndGames <- function(d1, formula) {
  list(
...
makeIndPlotGrid <- function(d1, i, pol) {
  ...
  # save
  ggsave(out, 
         file = paste0("figures/indGamesPlot", 
                       ifelse(pol == "SDO.T10", "SDO", "RWA"), ".pdf"), 
         height = 4, width = 7)
  return(out)
}

fitPolicy <- function(d, variable, coop, pun) {
  model <- '# measurement model
            coop =~ egame.PGG.T10 + egame.DG.T10 + egame.TG1.T10 + egame.TG2.T10 + egame.SH.T10
            pun  =~ egame.UG2.T10 + egame.TPP2.T10 + egame.SPP3.T10 + egame.SHP3.T10
            # control for comprehension
...
makePolicyGrid <- function(d1, sem1) {
  ...
  out <- ggdraw(out)
  # save
  ggsave(out, file = paste0("figures/policyPlotCoop.pdf"), 
         height = 5.5, width = 7)
  return(out)
}

makePolicyR2Grid <- function(d1) {
  ...
  # save
  ggsave(out, 
         file = paste0("figures/policyR2Plot.pdf"), 
         height = 4, width = 7)
  return(out)
}

makeTestRetestTable <- function(d2) {
  tibble(
    Game = 
      c("Trust Game (Give)", "Trust Game (Return)", "Public Goods Game", 
        "Dictator Game", "Ultimatum Game (Offer)", 
        "Ultimatum Game (Minimum Acceptable Offer)", 
        "Third-Party Punishment Game (Steal)",
        "Third-Party Punishment Game (Punish)",
        "Second-Party Punishment Game (Transfer)",
        "Second-Party Punishment Game (Punish Cooperator)",
        "Second-Party Punishment Game (Punish Defector)"),
    Correlation =
      c(
        cor(d2$egame.TG1.T10 , d2$egame.TG1.T11 , method = "spearman"),
        cor(d2$egame.TG2.T10 , d2$egame.TG2.T11 , method = "spearman"),
        cor(d2$egame.PGG.T10 , d2$egame.PGG.T11 , method = "spearman"),
        cor(d2$egame.DG.T10  , d2$egame.DG.T11  , method = "spearman"),
        cor(d2$egame.UG1.T10 , d2$egame.UG1.T11 , method = "spearman"),
        cor(d2$egame.UG2.T10 , d2$egame.UG2.T11 , method = "spearman"),
        cor(d2$egame.TPP1.T10, d2$egame.TPP1.T11, method = "spearman"),
        cor(d2$egame.TPP2.T10, d2$egame.TPP2.T11, method = "spearman"),
        cor(d2$egame.SPP1.T10, d2$egame.SPP1.T11, method = "spearman"),
        cor(d2$egame.SPP2.T10, d2$egame.SPP2.T11, method = "spearman"),
        cor(d2$egame.SPP3.T10, d2$egame.SPP3.T11, method = "spearman")
      ),
    ICC =
      c(
        icc(cbind(d2$egame.TG1.T10 , d2$egame.TG1.T11 ), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.TG2.T10 , d2$egame.TG2.T11 ), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.PGG.T10 , d2$egame.PGG.T11 ), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.DG.T10  , d2$egame.DG.T11  ), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.UG1.T10 , d2$egame.UG1.T11 ), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.UG2.T10 , d2$egame.UG2.T11 ), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.TPP1.T10, d2$egame.TPP1.T11), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.TPP2.T10, d2$egame.TPP2.T11), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.SPP1.T10, d2$egame.SPP1.T11), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.SPP2.T10, d2$egame.SPP2.T11), model = "twoway", type = "agreement", unit = "single")$value,
        icc(cbind(d2$egame.SPP3.T10, d2$egame.SPP3.T11), model = "twoway", type = "agreement", unit = "single")$value
      )
  )
}
...
fitRFModel <- function(d2RF, formula) {
  # fit model
...
fitPolicyRF <- function(d2, formula) {
...
getBeastImages <- function() {
  getBeastImage <- function(file) ggdraw() + draw_image(image_read(file), scale = 0.9)
  pA <- getBeastImage("images/beastImages/ants.jpg")
  pB <- getBeastImage("images/beastImages/bees.jpg")
  pC <- getBeastImage("images/beastImages/flamingos.jpg")
  pD <- getBeastImage("images/beastImages/cranes.jpg")
  pE <- getBeastImage("images/beastImages/crickets.jpg")
  out <- 
    plot_grid(
...
fitBEASTModel <- function(d2BEAST, formula) {
  # fit model
...
plotRFBEAST <- function(d2, beast1, rf1) {
  # fitted function
  ...
  out <- plot_grid(pA, pB, nrow = 1, labels = letters[1:2])
  # save
  ggsave(out, file = "figures/rfBeastPlot.pdf", height = 4, width = 8)
  return(out)
}

fitPolicyBEAST <- function(d2, formula) {
...
makePolicyGridBEAST <- function(d2, mA, mB, mC, mD, mE) {
  ...
  out <- ggdraw(out)
  # save
  ggsave(out, file = paste0("figures/policyPlotBEAST.pdf"), 
         height = 7, width = 7)
  return(out)
}

makeRFtable <- function(rf9) {
  summary(rf9)$fixed %>%
    as_tibble() %>%
    select(1:4) %>%
    mutate(
      IV = c("Intercept", "Round number", "RWA", "SDO", "Gender (Male = 1)", "Age", 
             "Deprivation", "Ethnicity (Maori)", "Ethnicity (Pacific)", "Ethnicity (Pakeha)", 
             "Education", "SES")
    ) %>%
    select(IV, everything()) %>%
...
makeBEASTtable <- function(beast9) {
  summary(beast9)$fixed %>%
    as_tibble() %>%
    select(1:4) %>%
    mutate(
      IV = c("Intercept", "RWA", "SDO", "Gender (Male = 1)", "Age", 
             "Deprivation", "Ethnicity (Maori)", "Ethnicity (Pacific)", 
             "Ethnicity (Pakeha)","Education", "SES")
    ) %>%
    select(IV, everything()) %>%
    rename(SE = Est.Error, `Lower 95% CI` = `l-95% CI`, `Upper 95% CI` = `u-95% CI`)
}

makeBEASTpolicyTable <- function(model) {
  summary(model)$fixed %>%
    as_tibble() %>%
    select(1:4) %>%
    mutate(
