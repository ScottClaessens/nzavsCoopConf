...
(ref:efaTableCaption) Factor loadings from an exploratory factor analysis of all items from the 16-item SDO scale (Sidanius & Pratto, 1999) and the 30-item RWA scale (Altemeyer, 1996). The factor model was fitted to a separate validation dataset of New Zealand participants (n = `r nrow(readd(dVal))`). The twelve items used in the New Zealand Attitudes and Values Study are emphasised in bold.
...
(ref:testRetestTableCaption) Test-retest reliability for the economic games in Peysakhovich et al. (2014) across Studies 1 and 2. Correlations are Spearman's rank correlations (all _p_ < .050). Intra-class correlations are two-way single score agreements across the two studies (all _p_ < .050).
...
Altemeyer, B. (1996). _The authoritarian specter_. London: Harvard University Press.
...
Sidanius, J., & Pratto, F. (1999). _Social dominance: An intergroup theory of social hierarchy and oppression_. Cambridge: Cambridge University Press.
